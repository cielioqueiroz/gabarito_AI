  ...
  return (
    <div className="flex items-center gap-3 rounded-xl border border-border bg-elevated/50 px-3 py-2">
      ...
      <button
        onClick={toggle}
        disabled={loading}
        className="flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-full bg-brand-solid text-white transition-[filter] hover:brightness-110 disabled:opacity-70 cursor-pointer"
      ...
      <div className="min-w-0 flex-1">
        ...
        <input
          ...
          onChange={seek}
          disabled={!dur}
          className="h-1 w-full cursor-pointer appearance-none rounded-full bg-border accent-brand disabled:cursor-default"
      ...
      <button
        ...
        className="flex-shrink-0 rounded-md border border-border px-2 py-1 font-mono text-[10px] text-muted-foreground transition-colors hover:border-brand/30 hover:text-brand cursor-pointer"
        title="Velocidade"
      >
        {speed}×
      </button>
      ...
      {started && !error && (
        <a
          href={src}
          download="resumo.mp3"
          className="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-md border border-border text-muted-foreground transition-colors hover:border-brand/30 hover:text-brand"
          title="Baixar MP3"
        >
          <Download size={14} />
        </a>

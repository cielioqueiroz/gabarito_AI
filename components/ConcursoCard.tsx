  ...
  return (
    <>
      <motion.div
        ...
        className="group bg-surface rounded-xl border border-border p-5 hover:border-[#F2D53C]/40 hover:shadow-lg hover:shadow-black/10 transition-colors duration-200"
      >
        <div className="flex items-start justify-between mb-4">
          <Link href={`/concurso/${concurso.id}`} className="flex-1 min-w-0 cursor-pointer">
            <h2 className="font-bold text-foreground text-base leading-tight tracking-tight truncate group-hover:text-[#F2D53C] transition-colors duration-150">
              {concurso.nome}
            </h2>
        ...
        <Link href={`/concurso/${concurso.id}`} className="flex items-center justify-between gap-4 cursor-pointer">
          <div className="space-y-2">
            <div className="flex items-center gap-2 text-xs text-muted">
              <span className="h-1.5 w-1.5 rounded-full bg-[#F2D53C]" />
